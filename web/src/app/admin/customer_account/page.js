...
export default function MyComponent(props) {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const response = await fetch('https://localhost:7159/api/Admin/GetCustomerList');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const handle_delete_Customer = async (customerId) => {
    try {
      const response = await axios.delete("https://localhost:7159/api/Users/DeleteCustomer", {
        headers: {
          'Content-Type': 'application/json', // Make sure to specify the content type
        },
        data: {
          UserId: customerId // This must match the structure expected by the server
        }
              ...
              <div className="flex flex-col grow items-start px-5 pt-5 pb-12 mx-auto w-full text-xs leading-3 bg-gray-800 max-md:mt-2.5" id="left_border">
                ...
                <div className="flex gap-1.5 mt-7 ml-3 tracking-wide text-white whitespace-nowrap max-md:ml-2.5">
                  <img
                    ...
                    className="aspect-square w-[18px]"
                  />
                  <button onClick={() => navigation("/admin/account")}> <div className="grow my-auto">Account</div></button>
                </div>
                                      ...
                                      {customeraccounts.map((item, index) => (
                                        <tr key={index} class={index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}>
                                          ...
                                          <td class="text-left py-3 px-4">
                                            <button onClick={()=>handle_delete_Customer(item.userId)} class="bg-red-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out">Delete User</button>
                                          </td>
