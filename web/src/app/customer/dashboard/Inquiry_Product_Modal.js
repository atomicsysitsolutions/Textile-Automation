...
const Inquiry_Modal = ({ isOpen, onClose, modalContent, onSubmit }) => {
  ...
  useEffect(() => {
    if (modalContent) {
      setFormData(prevState => ({
        ...
        Name: modalContent.fields.find(field => field.name === 'Name')?.value || '',
        Email: modalContent.fields.find(field => field.name === 'Email')?.value || '',
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('https://localhost:7159/api/Products/PostInquiry', formData);
      setSuccessMessage('Inquiry submitted successfully.');
      setError(null);
      onSubmit(formData); // Pass form data to parent component
      setIsInquiryModalOpen(false); // Close the modal after submission
    } catch (error) {
